...
void VGG16(engine::kind engine_kind){
        ...
        auto conv1_prim_desc = std::get<0>(conv1);
        auto conv1_src_memory = std::get<1>(conv1).at(DNNL_ARG_SRC);
        auto conv1_weights_memory = std::get<1>(conv1).at(DNNL_ARG_WEIGHTS);
        auto conv1_bias_memory = std::get<1>(conv1).at(DNNL_ARG_BIAS);
        auto conv1_dst_memory = std::get<1>(conv1).at(DNNL_ARG_DST);
        
        // Create the convolution primitive
        net.push_back((conv1_prim_desc));
